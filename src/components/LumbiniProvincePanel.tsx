            ...
            <tr key={label}>
              <td className="rautahat-cp-table__label">
                <span
                  ...
                  style={{ background: colors[label] ?? LUMBINI_CHART_COLORS.slate }}
                />
                {label}
              </td>
...
function AgePyramid() {
  const rows = [...lumbiniAgeDistribution2021]
...
export function LumbiniProvincePanel() {
  const genderColors: Record<string, string> = {
    Males: LUMBINI_CHART_COLORS.blue,
    Females: LUMBINI_CHART_COLORS.orange,
  }
  const ageGroupColors: Record<string, string> = {
    '0–14 years': LUMBINI_CHART_COLORS.blue,
    '15–64 years': LUMBINI_CHART_COLORS.green,
    '65+ years': LUMBINI_CHART_COLORS.orange,
  }
  const citizenshipColors: Record<string, string> = {
    Nepal: LUMBINI_CHART_COLORS.blue,
    India: LUMBINI_CHART_COLORS.orange,
    'Other Citizenship': LUMBINI_CHART_COLORS.slate,
  }
  const birthColors: Record<string, string> = {
    'Same local unit': LUMBINI_CHART_COLORS.blue,
    'Other local unit in same district': LUMBINI_CHART_COLORS.green,
    'Other district': LUMBINI_CHART_COLORS.orange,
    'Other country': LUMBINI_CHART_COLORS.red,
  }
  const literacyColors: Record<string, string> = {
    Yes: LUMBINI_CHART_COLORS.green,
    'Only reading': LUMBINI_CHART_COLORS.cyan,
    No: LUMBINI_CHART_COLORS.orange,
  }

  const genderSlices = rowsToSlices(lumbiniGender2021 as unknown as Record<string, number>, [
    LUMBINI_CHART_COLORS.blue,
    LUMBINI_CHART_COLORS.orange,
  ])
  const ageSlices = rowsToSlices(lumbiniBroadAge2021 as unknown as Record<string, number>, [
    LUMBINI_CHART_COLORS.blue,
    LUMBINI_CHART_COLORS.green,
    LUMBINI_CHART_COLORS.orange,
  ])
  const citSlices = rowsToSlices(lumbiniCitizenship2021 as unknown as Record<string, number>, [
    LUMBINI_CHART_COLORS.blue,
    LUMBINI_CHART_COLORS.orange,
    LUMBINI_CHART_COLORS.slate,
  ])
  const birthSlices = rowsToSlices(lumbiniPlaceOfBirth2021 as unknown as Record<string, number>, [
    LUMBINI_CHART_COLORS.blue,
    LUMBINI_CHART_COLORS.green,
    LUMBINI_CHART_COLORS.orange,
    LUMBINI_CHART_COLORS.red,
  ])
  const litSlices = rowsToSlices(lumbiniLiteracyAge5Plus2021 as unknown as Record<string, number>, [
    LUMBINI_CHART_COLORS.green,
    LUMBINI_CHART_COLORS.cyan,
    LUMBINI_CHART_COLORS.orange,
  ])

  return (
    <section className="rautahat-cp" aria-labelledby="lumbini-cp-title">
      <div className="rautahat-cp__inner">
        <nav className="rautahat-cp__crumb" aria-label="Breadcrumb">
          ...
          <span>Nepal</span>
          <span aria-hidden> → </span>
          <strong>{lumbiniProvinceMeta.name}</strong>
        </nav>

        <h2 id="lumbini-cp-title" className="rautahat-cp__h1">
          {lumbiniProvinceMeta.name.toUpperCase()}
        </h2>
        <p className="rautahat-cp__subtitle">
          {lumbiniProvinceMeta.status} · {lumbiniProvinceMeta.nameNative}
        </p>
          ...
          <div className="rautahat-cp__keystat">
            ...
            <dl className="rautahat-cp__keystat-list">
              <div>
                ...
                <dd>{lumbiniPopulationByCensus[lumbiniPopulationByCensus.length - 1].population.toLocaleString()}</dd>
              </div>
              <div>
                <dt>Transcription</dt>
                <dd>{lumbiniProvinceMeta.transcription}</dd>
              </div>
            </dl>
          </div>
            ...
            <table className="rautahat-cp__summary-table">
              <thead>
                <tr>
                  ...
                  <th>Status</th>
                  <th>Native</th>
                  {lumbiniPopulationByCensus.map((c) => (
              ...
              <tbody>
                <tr>
                  ...
                  <td>{lumbiniProvinceMeta.status}</td>
                  <td>{lumbiniProvinceMeta.nameNative}</td>
                  {lumbiniPopulationByCensus.map((c) => (
        ...
        <div className="rautahat-cp__cards">
          <DemographicCard title="Gender (C 2021)" pie={<PieChart slices={genderSlices} />}>
            <DataTableWithPct rows={{ ...lumbiniGender2021 }} colors={genderColors} />
          </DemographicCard>

          <DemographicCard title="Age Groups (C 2021)" pie={<PieChart slices={ageSlices} />}>
            <DataTableWithPct rows={{ ...lumbiniBroadAge2021 }} colors={ageGroupColors} />
          </DemographicCard>
          ...
          <DemographicCard title="Citizenship (C 2021)" pie={<PieChart slices={citSlices} />}>
            <DataTableWithPct rows={{ ...lumbiniCitizenship2021 }} colors={citizenshipColors} />
          </DemographicCard>

          <DemographicCard title="Place of Birth (C 2021)" pie={<PieChart slices={birthSlices} />}>
            <DataTableWithPct rows={{ ...lumbiniPlaceOfBirth2021 }} colors={birthColors} />
          </DemographicCard>

          <DemographicCard title="Literacy (A5+) (C 2021)" pie={<PieChart slices={litSlices} />}>
            <DataTableWithPct rows={{ ...lumbiniLiteracyAge5Plus2021 }} colors={literacyColors} />
          </DemographicCard>
        </div>

        <p className="rautahat-cp__footer-source">
          <strong>Reference:</strong> {LUMBINI_DATA_SOURCE_LABEL}{' '}
          <a href={LUMBINI_DATA_SOURCE_URL} target="_blank" rel="noopener noreferrer">
            citypopulation.de →
          </a>
        </p>
      </div>
